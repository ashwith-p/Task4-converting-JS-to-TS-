...
export enum pages{
    ...
    roles='roles.html',
    addRoles='addRoles.html',
    roleDescr='role-descr.html'
}

export enum styles{
    employee='css/employee.css',
    addEmployee='css/add_employee.css',
    roles='css/roles.css',
    addRoles='css/addRole.css',
    roleDescr='css/role-descr.css'
}

fetchEmployee(pages.employee,styles.employee);

export async function fetchEmployee(page: string,css:string){
    addStyle(css);
...
function operations(page:string){
    ...
    if(page=='employees.html')
    {
        ...
        (document.getElementById("html-content")!).addEventListener('click', function (e) {
            ...
            if ((e.target! as HTMLElement).id == 'filter-icon') {
                object.removeFilter();
            }
            else if ((e.target! as HTMLElement).className == 'add-btn') {
    ...
    if(page=='add-employee.html')
    {
        ...
        (document.getElementsByClassName('date-of-birth'))[0]!.addEventListener('blur', function (e) {
            addEmployeeObject.checkABove18(e.target! as HTMLInputElement);
        });
        (document.getElementById("html-content")!).addEventListener('click',async function(e){
            if(((e.target! as HTMLElement).tagName=="INPUT" || (e.target! as HTMLElement).tagName=="SELECT") && page=='add-employee.html')
            {
    ...
    if(page=='roles.html')
    {
        ...
        for(var i=0;i<document.getElementsByClassName("view-employee m-8").length;i++)
        {
            document.getElementsByClassName("view-employee m-8")![i].addEventListener('click',(e)=>{
                rolesObject.roleEmployees((e.target! as HTMLElement));
        ...
        for(var i=0;i<document.getElementsByClassName("edit-btn").length;i++)
        {
            document.getElementsByClassName("edit-btn")![i].addEventListener('click',(e)=>{
                rolesObject.editRoles((e.target! as HTMLElement));
        ...
        (document.getElementById("html-content")!).addEventListener('click',async function(e){
            ...
            if(((e.target!) as HTMLElement).className==' add-roles-btn'){
                  fetchEmployee(pages.addRoles,styles.addRoles);
            }
    ...
    if(page=='role-descr.html')
    {
        ...
        (document.getElementById("html-content")!).addEventListener('click',async function(e){
        if((e.target! as HTMLElement).className=='add-roledesc-btn'){
...
function addStyle(sheetName:string)
{
    ...
    {
        if(document.styleSheets[i].href=='http://127.0.0.1:1222/'+'css/common.css' || document.styleSheets[i].href=='http://127.0.0.1:1222/'+sheetName)
        {
